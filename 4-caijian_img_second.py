...
yao_id = "image_all_res/14"
folder_path = 'D:/code/juhua/test_yaocai_quanliucheng/test_quanliucheng/test/' + yao_id + '_twokind/yao_if_detection/1/'
count = 0
idx = 0
for filename in os.listdir(folder_path):
    if filename.endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
        ...
        # 初始化SLIC超像素
        slic = cv2.ximgproc.createSuperpixelSLIC(img, region_size=70, ruler=20.0)
        slic.iterate(100)
        # save_folder = "./_res/" + img_name
        save_folder = "D:/code/juhua/test_yaocai_quanliucheng/test_quanliucheng/test/" + yao_id + "_twokind/yao_second2"
        print(save_folder)
        ...
        for i in range(number_slic):
            ...
            if h_c > 100 and w_c > 100:
                continue
            if h_c > 40 and w_c > 40:
                # 显示或保存每个超像素的矩形区域
        ...
        img_slic = img.copy()
        cv2.drawContours(img_slic, contours, -1, (0, 0, 0), thickness=1, lineType=cv2.LINE_AA)  # 红色，线宽2
        cv2.imwrite(save_folder + "/img_slic_" + str(idx) + ".jpg", img_slic)
