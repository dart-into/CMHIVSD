...
def hog_hist(img_path, bins):
    ...
    print(type(image))
    image = cv2.resize(image, (90, 90), cv2.INTER_LANCZOS4)
    # 将原图转变为灰度图
    ...
    # bin_la2 = list(range(bins, 2*bins))
    # plt.bar(bin_la, prob_hist)  # bins参数决定了直方图的柱子数量
    # plt.bar(bin_la2, prob_hist2)  # bins参数决定了直方图的柱子数量
    # plt.ylim(0, 1.0)
    # # plt.plot(bin_edges[:20], prob_hist)  # bins参数决定了直方图的柱子数量
    # hist_filename = './hist_bin20_all/histogram_gray_' + folder_name + "_" + base_name + '.png'
...
# 创建 CSV 文件并写入表头
csv_filename = "./test_quanliucheng/test/" + yao_id + "_test_csv/test_nine_bin20_" + yao_id.split('/')[-1] + "_yao_second2.csv"
with open(csv_filename, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(
        ['image_name', 'mean2', 'mean2_gray', 'multi', 'if_det'])  # 表头
folder_path = 'D:/code/juhua/test_yaocai_quanliucheng/test_quanliucheng/test/' + yao_id +'_twokind/yao_second_twokind/yao/'
# # 获取文件夹中的所有文件名
images_files = []
actual_labels = []
...
# with open('D:/code/juhua/test_yaocai_quanliucheng/test_second_chaoxiangsu.csv', 'r', encoding='utf-8') as csvfile:  # 注意编码格式
#     reader = csv.DictReader(csvfile)
#     for row in reader:
#         # images_files.append('D:/code/juhua/Split_yao/test/' + row['image_name'])
#         images_files.append(row['image_name'])
...
ACC = 0
SUM = 0
target_base_dir = "D:/code/juhua/test_yaocai_quanliucheng/test_quanliucheng/test/" + yao_id + "_twokind/yao_if_detection/"
...
for image_file in images_files:
    ...
    image_re = cv2.resize(image, (90, 90), cv2.INTER_LANCZOS4)
    # 设置窗口大小
    window_size = 30
    bins = 20
    dis_p = 0.01
    ...
    # split_output_folder = './test_res/split_res8/'
    ...
    for i in range(0, height - window_size + 1, window_size):
        for j in range(0, width - window_size + 1, window_size):
            ...
            # plt.ylim(0, 1.0)
            # # 保存直方图为文件
            # hist_save_folder = "D:/code/juhua/test_yaocai_quanliucheng/test_quanliucheng/test/image2_res/62806-2-577_twokind/hist_bin20_all/"
            # os.makedirs(hist_save_folder, exist_ok=True)
            # plt.savefig(hist_save_folder + 'histogram_gray_' + folder_name + "_" + base_name + '_' + str(count) + '.png')
            # plt.close()
            # 保存切割的图片
    ...
    if mulit_res < 0.008:
        if_det = 0
        dest_dir = target_base_dir + '0_second/'
    else:
        if_det = 1
        dest_dir = target_base_dir + '1_second/'
    row = [image_file] + feature_res + feature_res_gray + [mulit_res, if_det]
    ...
    second_twokind_target_base_dir = target_base_dir
    no_detection_first_dir = second_twokind_target_base_dir + '0/'
    no_detection_second_dir = second_twokind_target_base_dir + '0_second/'
    bigimages_dir = second_twokind_target_base_dir + 'bigimage/'
    # 创建 bigimg 文件夹（如果不存在）
    os.makedirs(bigimages_dir, exist_ok=True)
    if os.path.exists(no_detection_first_dir):
        no_detection_first_images = [f for f in os.listdir(no_detection_first_dir) if
                                     f.endswith(('jpg', 'jpeg', 'png', 'bmp', 'gif'))]
        for fi in no_detection_first_images:
            image_origin_path = os.path.join(no_detection_first_dir, fi)
            bigimage_target_path = os.path.join(bigimages_dir, fi)
            shutil.copy(image_origin_path, bigimage_target_path)
    if os.path.exists(no_detection_second_dir):
        no_detection_second_images = [f for f in os.listdir(no_detection_second_dir) if
                                      f.endswith(('jpg', 'jpeg', 'png', 'bmp', 'gif'))]
        for fi in no_detection_second_images:
            image_origin_path = os.path.join(no_detection_second_dir, fi)
            bigimage_target_path = os.path.join(bigimages_dir, fi)
            shutil.copy(image_origin_path, bigimage_target_path)
